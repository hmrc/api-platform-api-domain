...
class ApiIdentifierSpec extends BaseJsonFormattersSpec with Inspectors {
  val aContext       = ApiContext("c1")
  val aVersionNbr       = ApiVersionNbr("1.0")
  val anApiIdentifer = ApiIdentifier(aContext, aVersionNbr)
  ...
  "ApiVersion" should {
    "convert to json" in {
      ...
      Json.toJson(aVersionNbr) shouldBe JsString("1.0")
    }

    "read from json" in {
      testFromJson[ApiVersionNbr](""""1.0"""")(aVersionNbr)
    }
    ...
    "generate a random version" in {
      val vs = Range(1, 1000).map(_ => ApiVersionNbr.random).map(_.value)
  ...
  "ApiIdentifier" should {
    ...
    "sort according to context and version" in {
      val i1 = ApiIdentifier(ApiContext("alice"), ApiVersionNbr("1.0"))
      val i2 = ApiIdentifier(ApiContext("alice"), ApiVersionNbr("2.0"))
      val i3 = ApiIdentifier(ApiContext("bob"), ApiVersionNbr("1.0"))
      val i4 = ApiIdentifier(ApiContext("bob"), ApiVersionNbr("1.1"))
